...
def viterbi(HMM, ev):
    ...
    for i in range(1, t):
        print("m["+str(i)+"] = "+str(HMM.sensor_dist(ev[i+1]))+
              " * [max("+str(HMM.transition_model[0][0])+"*"+str(m[i-1][0])+"), "+str(HMM.transition_model[0][1])+"*"+str(m[i - 1][1])+
              "), max("+
              str(HMM.transition_model[1][0])+
              "*"+
              str(m[i-1][0])+
              "), "+
              str(HMM.transition_model[0][1])+
              "*"+
              str(m[i - 1][1])+
              ")]")
